...
def parse_ticker(ticker: str) -> dict:
    ...
    if isinstance(sec, Security):
        ...
        d['ticker_occ'] = FORMATS_FOR_REBUILD[sec.asset_class][FORMAT_TYPES.OCC].to_ticker_string(sec)
        d['ticker_bloomberg'] = FORMATS_FOR_REBUILD[sec.asset_class][FORMAT_TYPES.Bloomberg].to_ticker_string(sec)
        d['ticker_eze'] = FORMATS_FOR_REBUILD[sec.asset_class][FORMAT_TYPES.Eze].to_ticker_string(sec)
        return d
    else:
